...
### 2-4. サブネットのサイズが十分に確保されているか

Standard Logic Apps で VNet 統合を利用する場合、**統合に使用するサブネットのサイズ不足**により、ランタイム エラーや接続エラーが発生することがあります。

VNet 統合に使用しているサブネットについて、**利用可能な IP アドレスが十分残っているか**をご確認ください。

また、Standard Logic Apps の仮想ネットワーク統合用のサブネットには、以下の制約があります。

- 仮想ネットワーク統合用サブネットの **最小サイズは /27（32 アドレス）**
- サブネット容量に関する問題を回避するため、**/26（64 アドレス）の使用が推奨**
- **一度割り当てた後に、サブネット サイズを変更することはできない**

Standard Logic Apps は、スケールアウトや内部処理によりサブネット内の IP アドレスを動的に消費します。  
そのため、将来的なスケールも考慮し、十分なサイズのサブネットを事前に確保することが重要です。

[標準ワークフローと仮想ネットワーク間のトラフィックをセキュリティで保護する - Azure Logic Apps | Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/logic-apps/secure-single-tenant-workflow-virtual-network-private-endpoint#considerations-for-outbound-traffic-through-virtual-network-integration)

[アプリを Azure 仮想ネットワークと統合する - Azure App Service | Microsoft Learn #サブネットの要件](https://learn.microsoft.com/ja-jp/azure/app-service/overview-vnet-integration#subnet-requirements)
