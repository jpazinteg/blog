...
Standard Logic Apps で VNet 統合を利用する場合、統合に使用するサブネットのサイズ不足により、ランタイム エラーや接続エラーが発生することがあります。

VNet 統合に使用しているサブネットについて、利用可能な IP アドレスが十分残っているかをご確認ください。

また、Standard Logic Apps の仮想ネットワーク統合用のサブネットには、以下の制約があります。

- 仮想ネットワーク統合用サブネットの最小サイズは /27（32 アドレス）
