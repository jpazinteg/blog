...
アクセスが失敗している場合には、以下のような要因が考えられます。
1. Logic Apps の VNet に構成されている DNS に問題がある
A. カスタム DNS を作成したが、正しい名前解決に必要な構成が不足している
2. Storage Account の Private Endpoint に DNS が構成されていない
3. Storage Account の Private Endopoint に構成されている DNS に問題がある
A. 既定値で作成したが、意図とは異なる仮想ネットワーク (VNet) リソースを使用している
B. カスタム DNS を作成したが、正しい名前解決に必要な構成が不足している

>[!NOTE]
>カスタムまたはオンプレミスの DNS サーバーを使用している場合は、privatelink サブドメインのストレージ アカウント名をプライベート エンドポイントの IP アドレスに解決するように DNS サーバーを構成する必要があります。 これを行うには、 privatelink サブドメインを仮想ネットワークのプライベート DNS ゾーンに委任するか、DNS サーバーで DNS ゾーンを構成し、DNS A レコードを追加します。

Private Endpoint および DNS については下記資料をご参照ください。

[プライベート エンドポイントを使用する - Azure Storage | Microsoft Learn # プライベート エンドポイントの DNS の変更](https://learn.microsoft.com/ja-jp/azure/storage/common/storage-private-endpoints#dns-changes-for-private-endpoints)
[Azure Virtual Network 名前解決ガイド | Microsoft Learn # 独自の DNS サーバーを使用する名前解決](https://learn.microsoft.com/ja-jp/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances?tabs=redhat#name-resolution-that-uses-your-own-dns-server)
[Azure プライベート エンドポイントのプライベート DNS ゾーンの値 | Microsoft Learn](https://learn.microsoft.com/ja-jp/azure/private-link/private-endpoint-dns)
