...
**[記憶分析]** より、インスタンスあたりのメモリ使用量をご確認いただけます。
...
孤立した実行は検出されない限りリソースを消費し続け、CPU やメモリを圧迫し、パフォーマンス低下の原因となります。
...
OOM が発生すると、該当実行が孤立状態になり、ランタイムは内部的に複数回の再試行を行いますが、状態が回復不能な場合はワーカーが再びクラッシュすることもあります。
...
なお、1 つの ASP 上に複数の Logic Apps を配置している場合、Logic Apps 側の [記憶分析] でインスタンスごとの詳細を確認することで、どの Logic App がより負荷を与えているかなどより正確に分析いただけます。
